    ...
    try {
      const response = await fetch('http://localhost:3000/agents/items', {
        ...
        body: JSON.stringify({
          ...
          seller_id: formData.sellerId || 'default-seller'
        })
      });
      ...
      if (response.ok && data.success) {
        setMessage({ type: 'success', text: `Item "${formData.name}" registered successfully!` });
        setFormData({ name: '', description: '', price: '', sellerId: '' });
        fetchItems(); // Refresh item list
      } else {
        setMessage({ type: 'error', text: data.error || 'Failed to register item' });
      }
    } catch (error) {
